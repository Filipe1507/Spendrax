...
## 4️⃣ EXEMPLO PRÁTICO: DASHBOARD

### Fluxo de dados completo

```
┌─────────────────────────────────────────────────────┐
│ Dashboard monta (useEffect no componente)           │
└──────────────────┬──────────────────────────────────┘
                   │
                   ▼
    ┌──────────────────────────────┐
    │ fetchData() faz 3 chamadas   │
    ...
    GET /transactions/summary/monthly?month=3&year=2026
    ...
    GET /categories/
        ...
        Renderiza Dashboard com dados
                   │
                   ▼
    Utilizador interage (adiciona/edita/elimina)
                   │
        ┌──────────┴──────────┐
        ▼                     ▼
    Clica em "Nova"    Clica em "✏️" (editar)
        │                     │
        ▼                     ▼
    api.post(               api.put(
...
**Comportamentos:**
- ❌ Network error → catch genérico
- ❌ 401 Unauthorized → redireciona `/login`
- ❌ Outro erro → mostra `alert()` ou estado de erro

---

## 8️⃣ FLUXO DE DADOS VISUAL
...
┌────────────────┐                       │
│ api.post()  ────────────────────────►│
│ com dados       │   POST /transactions│
└────────┬───────┘     + JSON body     │
         │ ◄────────────────────────────►│
         │                        Created│
         ▼                                │
   fetchData()  ─────────────────────────►│
  (recarrega)        GET /transactions/   │
```

---

## 9️⃣ RESUMO INTEGRAÇÃO POR TIPO

| Integração | Como | Exemplo |
|-----------|------|---------|
| **Backend REST** | `api.get/post/put/delete()` | `/transactions/`, `/categories/` |
| **Autenticação** | JWT em localStorage + interceptador | `Bearer {token}` auto-adicionado |
| **IA (Groq)** | `fetch()` direto para API externa | Parse linguagem natural |
| **Dados Locais** | localStorage | Token, user data |
| **Roteamento** | React Router + PrivateLayout | Proteção de rotas |
| **Estado** | useState + useCallback + useEffect | Fetch ao montar |

---

## 🔟 ⚠️ PROBLEMAS DE SEGURANÇA IDENTIFICADOS

### 1. Chave Groq hardcoded (AIChat.tsx:23)

```typescript
const GROQ_API_KEY = import.meta.env.VITE_GROQ_API_KEY;
```

**Risco:** Chave exposta publicamente, qualquer um pode usar
**Solução:** Criar proxy no backend, chamar `/api/ai/parse` em vez de Groq direto

### 2. JWT em localStorage

**Risco:** Vulnerável a XSS
**Solução:** Usar HttpOnly cookies em vez de localStorage

### 3. Sem tratamento de refresh tokens

**Risco:** Quando token expira, utilizador fica preso
**Solução:** Implementar refresh token rotation

### 4. Sem tratamento global de 401

**Risco:** Se token expira, cada página mostra erro individual
**Solução:** Interceptador global no axios para lidar com 401
...
### Transações
- `GET /transactions/summary/monthly?month=X&year=Y` - Resumo mensal
- `GET /transactions/?month=X&year=Y` - Lista de transações
- `POST /transactions/` - Criar transação
- `PUT /transactions/{id}` - Editar transação
- `DELETE /transactions/{id}` - Eliminar transação
...
### Orçamentos
- `GET /budgets/?month=X&year=Y` - Listar orçamentos
- `POST /budgets/` - Criar orçamento
- `DELETE /budgets/{id}` - Eliminar orçamento
